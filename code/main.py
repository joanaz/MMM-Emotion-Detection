...
while rval:
  ...
  cv.SaveImage(path_to_file + '/webcam.jpg', cv.fromarray(frame))
  img = cv.LoadImage(path_to_file + '/webcam.jpg')  # input image
  mouth = m.findmouth(img)
  # show(mouth)
  if mouth != 2:  # did not return error
    mouthimg = crop(mouth)
    cv.SaveImage(path_to_file + '/webcam-m.jpg', mouthimg)
    # predict the captured emotion
    result = lr.predict(vectorize(path_to_file + '/webcam-m.jpg'))
    if result == 1:
    ...
    # error message
    to_node('error', {
            'err_msg': 'failed to detect mouth. Try hold your head straight and make sure there is only one face.'})
